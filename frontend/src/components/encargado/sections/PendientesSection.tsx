...
export default function PendientesSection({ onNavTo }: Props) {
  const { solicitudes, isLoading, isRefreshing, error, refetch } = useMisPendientes();
...
function SolicitudPendienteCard({ solicitud }: { solicitud: SolicitudRenta }) {
  return (
    <div className="bg-white border border-slate-200 border-l-4 border-l-amber-400 rounded-lg shadow-md overflow-hidden">
      ...
      {/* Footer — observaciones (solo si tiene) */}
      {solicitud.notas && (
...
function EmptyState({ onNavTo }: { onNavTo?: (s: string) => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4">
      ...
      <div className="text-center space-y-1">
        <p className="text-sm font-semibold text-slate-600">Sin solicitudes pendientes</p>
        <p className="text-xs text-slate-400 max-w-xs leading-relaxed">
          No tienes solicitudes esperando aprobación en este momento.
        </p>
      </div>
      {onNavTo && (
        <button
