...
export default function RechazadasTab() {
  ...
  // Selector estable: referencia directa al array. El filter se hace en useMemo
  // para evitar crear un array nuevo en cada llamada a getSnapshot (Zustand v5
  // con useSyncExternalStore lanzaría "unstable snapshot" si el selector
  // devuelve una nueva referencia en cada llamada aunque los datos no cambien).
  const allSolicitudes  = useSolicitudesStore(s => s.solicitudes);
  const storeRechazadas = useMemo(
    () => allSolicitudes.filter(s => s.estado === 'RECHAZADA'),
    [allSolicitudes],
  );
  ...
  const cargarPrimera = useCallback(async (desde: string, hasta: string) => {
    ...
    setNextCursor(null);
    try {
      const res = await solicitudesService.getRechazadas({
...
function SolicitudRechazadaCard({ solicitud }: { solicitud: SolicitudRenta }) {
  return (
    <div className="bg-white border border-slate-200 border-l-4 border-l-red-400 rounded-lg shadow-md overflow-hidden">
      ...
      {/* Footer — motivo de rechazo + observaciones */}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-400">
      ...
      <p className="text-xs text-center max-w-xs leading-relaxed">
        No hay solicitudes rechazadas en el rango de fechas seleccionado.
      </p>
    </div>
  );
}
