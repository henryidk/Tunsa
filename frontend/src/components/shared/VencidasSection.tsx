...
import RentaVencidaCard from './RentaVencidaCard';
import RentaVencidaPesadaCard from './RentaVencidaPesadaCard';
...
export default function VencidasSection({
  ...
  fetchSolicitudes, showEncargado = false, initialFolio, onNavTo,
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState hayFiltro={busqueda.trim().length > 0} emptyText={emptyStateText} />
      ) : (
...
function EmptyState({ hayFiltro, emptyText }: { hayFiltro: boolean; emptyText: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
      ...
      <p className="text-xs text-center max-w-xs leading-relaxed">
        {hayFiltro ? 'Intenta con otro folio o nombre de cliente.' : emptyText}
      </p>
    </div>
  );
}
