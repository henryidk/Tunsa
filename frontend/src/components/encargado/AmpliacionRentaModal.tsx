...
// ── Inputs de duración (compartido entre modos) ───────────────────────────────
...
export default function AmpliacionRentaModal({
  ...
  const [estado, setEstado] = useState<Record<string, ItemExtension>>(
    () => Object.fromEntries(
      solicitud.items.map(item => [
        itemRef(item),
        // Si hay un solo ítem, queda seleccionado implícitamente
  ...
  const handleDuracion = (ref: string, raw: string) => {
    if (raw === '') {
      setEstado(prev => ({ ...prev, [ref]: { ...prev[ref], duracion: '' } }));
      return;
    }
    const n = parseInt(raw, 10);
    if (!isNaN(n) && n >= 1) {
      setEstado(prev => ({ ...prev, [ref]: { ...prev[ref], duracion: n } }));
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto">
          ...
          {confirmando ? (
            ...
            /* ── Modo ítem único: sin checkbox, inputs directos ── */
            (() => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      {itemKindLabel(item)} • Inicio: {item.fechaInicio}
                    </p>
                  </div>
                  {/* Inputs directos */}
                  <DuracionInputs
                    ext={ext}
                    onDuracion={val => handleDuracion(ref, val)}
                    onUnidad={val => handleUnidad(ref, val)}
                  />
                  <p className="text-[11px] text-slate-400">
                    El costo se calculará con los precios actuales al confirmar.
                  </p>
                </div>
              );
            })()
          ) : (
            /* ── Modo multi-ítem: checkboxes ── */
                ...
                return (
                  <div
                    ...
                    className={`border rounded-xl p-4 transition-colors ${
                      ext.checked ? 'border-indigo-300 bg-indigo-50/50' : 'border-slate-200 bg-white'
                    }`}
                  >
